...
- The extension lives in [`my-app/`](.) (Vite + `@crxjs/vite-plugin`), with:
  - `manifest.json`
  - popup entrypoint
  - background service worker
  - blocked/interstitial page
  - shared extension utilities
- **Status (simple slice):** popup reads tabs + summary + session goal, background redirects blocklisted hosts while a session is active, blocked page is `public/blocked.html`, styles import [`web-app/app/globals.css`](../web-app/app/globals.css). See [`README.md`](README.md). Optional Convex via `VITE_CONVEX_URL`.
